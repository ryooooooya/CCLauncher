...
CLAUDE.md には以下のゲートだけを追記する。手順の本体はこのファイルに置き、
ゲート発動時に読みに来させる（CLAUDE.md の行数を消費しないため）。
...
- 実装完了をユーザーに報告する前に、必ず Codex でコードレビューを行う
- 手順・コマンド・指摘の判断基準は `.claude/docs/base_codex_review.md` を読んで従う
```
...
```
現在の CLAUDE.md を読んで、base_codex_review.md の「CLAUDE.md に追記するセクション」にある
ゲート3行を既存の構成に自然に馴染む形で追記してください。
手順本体は追記せず、.claude/docs/base_codex_review.md への参照のままにしてください。
