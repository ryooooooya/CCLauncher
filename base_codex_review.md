...
ゲートは `base_dev_pipeline.md` の「常時適用ルールに追記するもの」に一本化した
（置き場所は `.claude/rules/project_conventions.md`。CLAUDE.md 本体には書かない）。
このファイルへのゲートを個別に置かず、パイプラインのゲート経由で読みに来させる。
CLAUDE.md 側は読み分け表に「Codex CLI のコマンド・レビュー判断基準 → このファイル」の行だけを持つ。
...
実装は実装エージェント（Codex）が行うため、コードレビューの本線は PR での `@codex review`
（下の節）になる。PR を経由しない変更のための代替経路として CLI でもレビューできる。

前提として守る不変条件は「レビューは実装と別インスタンスで行う」こと（`base_dev_pipeline.md`）。
実装セッションの `resume` でレビューさせない。自分の成果物を弁護する方向に働くため、必ず新規セッションで走らせる。

1. 実装をコミットする（サンドボックス下ではコミットできないため、人間または設計エージェントが行う）
2. **新規セッション**でコミットハッシュを指定して Codex にレビューさせる
3. 指摘を判断基準（下記）に照らして採否を決める。判断は設計エージェントまたは人間が行う
4. 採用した指摘の修正は実装エージェントに戻す。修正後はレビューセッション側で `resume --last` して再レビューする
   （引き継ぐのはレビューの文脈。実装セッションの文脈ではない）
...
ただし `AGENTS.md` は生成物なので直接書かない。観点のソースは
`.claude/rules/project_conventions.md` の「レビュー観点」節（雛形は `base_dev_pipeline.md`）で、
`/agents-md` の再生成で `AGENTS.md` に展開される。生成仕様は `base_agents_md.md`。
