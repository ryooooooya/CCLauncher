...
このチェックリストは常時適用ルールではなく、**節目に走らせる監査**の判定ソース。
実行は `/ux-audit`（セットアップは `base_ux_audit.md`）で行い、結果をレポートとして残す。
再実行して差分を見ることで、変更後の劣化を検出する。
UI 実装中に参照してもよいが、その場合も「実装しながら守る」対象は
プロダクト固有の ui spec のほうで、このファイルは一般原則側にあたる。
