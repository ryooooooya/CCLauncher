> Technology-specific guidance: [recipes/npm-security.md](recipes/npm-security.md) and [recipe index](recipes/README.md).

# base_security_npm

> Legacy移行資料: 新coreの原則は [dependencies](standards/dependencies.md) を参照。以下の旧ルール・実装例は新規baselineへ適用せず、技術固有の移行と旧文書削除は後続Issueで行う。
...
## 7. CI / GitHub 側の自動防御層とインシデント対応

- リポジトリ側の自動防御層（Dependabot, Dependency Review, CI audit, Actions の SHA ピン留め）の初期設定手順は `.claude/docs/base_security_npm_setup.md` を参照。プロジェクト初期化時に一度だけ実行する
- ユーザーからサプライチェーンインシデントの情報（「○○パッケージがやられたらしい」「このCVEは影響ある？」等）を受けたら、`.claude/docs/base_security_npm_incident.md` を読んで影響確認を実行する
