...
ユーザーはミリ秒ではなく「応答性」を感じる。同じ総時間でも、開始のレスポンスが速ければ「瞬時」に感じ、開始がもたつけば「ラグい」と感じる。実測速度を上げる前に、まず「速く感じる」設計を優先する。

- `response-first` — 最初の100〜200msのレスポンス感がアニメーション全体の印象を決める。総時間の短縮より、開始の反応速度を優先する
- `front-loaded-motion` — 動きは前半に集中させる。ease-out（速く始まって減速）が基本パターン。ease-in や linear は「最初が遅い」ため即時フィードバックに向かない
- `immediate-acknowledgment` — ユーザー操作に対しては、重い処理の開始前に軽量なフィードバック（プレスフィードバック、スケルトン、プレースホルダー）を先出しする。処理完了を待たせない
- `perceived-vs-actual` — 2秒かかる処理でも、開始の0〜200msで視覚応答があれば「速い」と感じる。逆に、開始が遅ければ総時間が短くても「遅い」と感じる
- `no-dead-time` — 操作からフィードバックまでの無反応時間をゼロに近づける。ネットワーク待ち・計算待ちの間も UI は応答し続ける
